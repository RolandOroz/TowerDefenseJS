...
function handleEnemies() {
    for (let i = 0; i < enemies.length; i++) {
        ...
        if (enemies[i].health <= 0) {
            let gainedResources = enemies[i].maxHealth;
            floatingMessages.push(new floatingMessage('+' + gainedResources, enemies[i].x, enemies.y, 30, 'black'));
...
function handleGameStatus() {
    context.fillStyle = 'gold';
    context.font = '30px Orbitron';
    context.fillText('Scores: ' + score, 20, 40);
    ...
    if (score >= winningScore && enemies.length === 0) {
        context.fillStyle = 'black';
        context.font = '90px Orbitron';
        context.fillText('WINNER', 230, 330);
    }
}
...
canvas.addEventListener('click', function () {
    ...
    if (numberOfResources >= defenderCost) {
        ...
        floatingMessages
            .push(new floatingMessage('need more resources', mouse.x, mouse.y, 20, 'blue'));
    }
});
